...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Have I Been Pwned');
}
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'check_password':
      return checkPassword(reqStr(args, 'password', '"correct horse battery staple" (the actual password to check — k-anonymity protects it)'));
    case 'check_password_prefix':
      return checkPasswordPrefix(reqStr(args, 'sha1_prefix', '"5BAA6" (first 5 hex chars of SHA-1)'));
    case 'list_breaches':
      return listBreaches(args.domain as string | undefined);
    case 'get_breach':
      return getBreach(reqStr(args, 'name', '"Adobe" or "LinkedIn"'));
    case 'list_data_classes':
      return listDataClasses();
    case 'check_account':
      return checkAccount(
        (args._apiKey as string | undefined)?.trim(),
        reqStr(args, 'account', '"user@example.com" (email or username)'),
...
async function fetchPwdRange(prefix: string): Promise<Map<string, number>> {
  const res = await pwFetch(`${PWNED_PWD_BASE}/range/${prefix}`, {
    headers: { 'User-Agent': USER_AGENT },
  });
  if (!res.ok) {
    throw await httpError(res, 'HIBP pwned-passwords error');
  }
...
async function hibpGet<T>(path: string, apiKey?: string): Promise<T> {
  ...
  const res = await pwFetch(`${HIBP_BASE}${path}`, { headers });
